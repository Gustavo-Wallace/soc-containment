...
func _draw() -> void:
	for link: PackedStringArray in links:
		...
		var blocked := blocked_device_id != "" and (link[0] == blocked_device_id or link[1] == blocked_device_id)
		var color := Color(VisualStyle.MUTED_TEXT, 0.48) if blocked else (VisualStyle.CONNECTION_SELECTED if highlighted else VisualStyle.CONNECTION)
		var line_width := 2.1 if highlighted else 1.35
		draw_line(a, b, color, line_width, true)
		if blocked:
			var middle := a.lerp(b, 0.5)
			draw_line(middle + Vector2(-7, -7), middle + Vector2(7, 7), VisualStyle.AMBER, 1.5)
			draw_line(middle + Vector2(-7, 7), middle + Vector2(7, -7), VisualStyle.AMBER, 1.5)
